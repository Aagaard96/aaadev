...
export default function ProjectCard({ title, description, occupation, img, position, link }) {
    return (
        <div
            className={`w-[800px] h-[250px] p-8 bg-gray-800 shadow-[0_7px_18px_#000] mb-6 rounded-lg text-white flex gap-10 group
        ...
        >
            {position === 'left' ? (
                <>
                    ...
                    {/* Image on the right */}
                    <div className="relative flex-1 group-hover:scale-105 duration-300">
                        <Image src={img} fill alt={title} className="object-cover rounded-lg border border-neutral-300 shadow-[0_7px_14px_#000] " />
                    </div>
                </>
            ) : (
                <>
